...
def main():
    ...
    for message in messages:
        #print(message['id'])
        completeMessage = service.users().messages().get(userId='me', id = messages['id']).execute()
        ...
        MessageTo = list(filter(lambda h: h['name']=='messageTo',headers))[0]['value']
        messageFrom = list(filter(lambda h: h['name']=='MessageFrom',headers))[0]['value']
        print(MessageTo)
